...
class Job {
    ...
    static async findAll(searchParms = {}) {
        ...
        const { title, minSalary, hasEquity } = searchParms;
        const whereConditions = [];
        const whereValues = [];

        if (title !== undefined) {
            whereValues.push(`%${title}%`)
            whereConditions.push(`title ILIKE $${whereValues.length}`)
        }
        if (minSalary !== undefined) {
            whereValues.push(minSalary)
            whereConditions.push(`salary >= $${whereValues.length}`)
        }
        if (hasEquity) {
            whereValues.push(0)
            whereConditions.push(`equity > $${whereValues.length}`)
        }
     ...
     *
     * Returns { id, title, salary, equity, company_handle, jobs }
